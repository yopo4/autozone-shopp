...
@section('title','AUTOZONE-SHOP || Page Panier')
                                ...
                                @if (Helper::getAllProductFromCart()->isNotEmpty())
                                    @foreach (Helper::getAllProductFromCart() as $key => $cart)
                                        <tr>
                                            ...
                                            <td class="qty" data-title="Qty"><!-- Commande d'entrée -->
                                            ...
                                            <td class="total-amount cart_single_price" data-title="Total"><span
                                                    class="money">{{ $cart['amount'] }} DH</span></td>
                                ...
                                @else
                                    <tr>
                                        <td class="text-center">
                                            Il n'y a aucun produit dans le panier. <a href="{{ route('product-grids') }}"
                                                style="color:blue;">Continuer vos achats</a>
                                ...
                                <div class="left">
                                    <div class="coupon">
                                        <form action="{{ route('coupon-store') }}" method="POST">
                                            @csrf
                                            <input name="code" placeholder="Entrez votre code de coupon">
                                ...
                                <div class="right">
                                    <ul>
                                        <li class="order_subtotal" data-price="{{ Helper::totalCartPrice() }}">Total du
                                            panier<span>{{ number_format(Helper::totalCartPrice(), 2) }} DH</span></li>

                                        @if (session()->has('coupon'))
                                            <li class="coupon_price" data-price="{{ Session::get('coupon')['value'] }}">
                                                Vous
                                                économisez<span>{{ number_format(Session::get('coupon')['value'], 2) }} DH</span>
                                            </li>
                                        @endif
                                        @php
                                        ...
                                        @endphp
                                        @if (session()->has('coupon'))
                                            <li class="last" id="order_total_price">Vous
                                                payez<span>{{ number_format($total_amount, 2) }} DH</span></li>
                                        @else
                                            <li class="last" id="order_total_price">Vous
                                                payez<span>{{ number_format($total_amount, 2) }} DH</span></li>
                                        @endif
                                    ...
                                    <div class="button5">
                                        <a href="{{ route('checkout') }}" class="btn">Confirmer</a>
